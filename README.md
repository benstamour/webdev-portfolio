...
[Lettrlink](https://github.com/benstamour/lettrlink) (Vue.js, AJAX, PHP, SQL, JavaScript) - 2025
------------------------------------------------

This repository features code for a game where the goal is connect letter tiles to spell words before the timer runs out. Each time the player spells a word, the tiles used in that word are replaced with new tiles. The game also includes gem tiles that provide score boosts or extra time, which are given when the player spells long or high-scoring words, and other special tiles, such as ones that randomize their letter each turn, can be used multiple times in a word, and more. Players can choose from one of three difficulty levels and 15 grid shapes, and they can also make custom games where additional settings can be chosen, like the amount of time given and the frequencies of special tiles. Custom game seeds can also be copied and sent to friends to compare scores when playing the same game board. The game can be played with a mouse or touchscreen, and a toggle between light mode and dark mode is provided. The code in this repository was built using Vue.js, JavaScript, CSS, HTML, and Bootstrap, and the website also uses AJAX and PHP. The game is playable **[here](https://lettrbox.kesug.com/link)**, where you can also play a special daily challenge and join the leaderboard!
